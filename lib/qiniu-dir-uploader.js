...
QiniuDirUploader.prototype.prefetchFile = function(fileName) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    cdnManager.prefetchUrls(urls, responseHandlerFactory(resolve, reject,urls));
  });
};
/**
 * @param {string} dir
  ...
  this.readDir(conf.buildAssetsRoot, function(filePath) {
    ...
    var prefetchFile = function() {
      self
        .prefetchFile(key)
        .then(function(urls) {
          signale.success('[Prefetch files] "' + urls.join(',') + '" successful');
        })
        .catch(function(err) {
    ...
    var onUploadSuccess = function() {
      ...
      if (conf.refresh) {
        self
          ...
          .then(function(urls) {
            signale.success('[Refresh files] "' + urls.join(',') + '" successful');
