...
QiniuDirUploader.prototype.refreshFileCache = function(fileName) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if(self.config.debugger){
      signale.debug('[Refresh files] '+refreshUrls.join(','))
    }
...
QiniuDirUploader.prototype.prefetchFile = function(fileName) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if(self.config.debugger){
      signale.debug('[Prefetch files] '+urls.join(','))
    }
  ...
  this.readDir(conf.buildAssetsRoot, function(filePath) {
    ...
    var prefetchFile = function() {
      self
        .prefetchFile(key)
        .then(function() {
          signale.info('Prefetch file: "' + key + '" successful');
        })
        .catch(function(err) {
    ...
    var onUploadSuccess = function() {
      ...
      if (conf.refresh) {
        self
          ...
          .then(function() {
            signale.info('Refresh file: "' + key + '" successful');
...
function responseHandlerFactory(resolve, reject,debug) {
  ...
  return function(err, respBody, respInfo) {
    ...
    if(debug){
      signale.debug(JSON.stringify(respBody));
      signale.debug(JSON.stringify(respInfo));
    }
