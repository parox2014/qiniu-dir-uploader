...
function QiniuDirUploader(conf) {
    this.config = conf;
    if(typeof this.config.keep!=='boolean'){
        this.config.keep=true;
    }
      ...
      fs.stat(filePath, function(err, stat) {
        ...
        files.forEach(function (file) {
            ...
            fs.stat(filePath, function (err, stat) {
                ...
                if (conf.excludes && conf.excludes.indexOf(extname) > -1) {
                    return;
                }
...
QiniuDirUploader.prototype.upload = function () {
    ...
    this.readDir(conf.buildAssetsRoot, function (filePath) {
        ...
        if (conf.keep) {
            key = finalPath.replace(conf.buildAssetsRoot, conf.publishAssetsRoot)
        } else {
            key = conf.publishAssetsRoot + '/' + getFileName(finalPath)
        }
...
/**
 *
 * @param conf {import('../types/upload-options').UploadOption}
 * @returns {QiniuDirUploader}
 */
